...
docker build -t "$TAG" -f /tmp/Dockerfile .
```

build 失败时：打印 stderr 末段（最多 200 行），**立即终止**，**不得**继续上传。

### 3b.3 本地 run + healthcheck
...
1. 打印 `docker logs <container>` 末段（≤200 行）
2. `docker stop <container>` + `docker rmi $TAG` + `rm /tmp/Dockerfile`
3. **立即终止**，**不得**继续上传
...
```bash
docker stop "${TAG%:*}-run"
docker save "$TAG" | gzip -1 > /tmp/showcase-image.tar.gz
```
...
- `healthcheck_path=<3b.3 中使用的 path>`

> 后端分支**不**生成 `/tmp/dist.zip`、**不**携带 `site_zip_file`。

### 3b.6 收尾（仅在步骤 8 成功 / 失败后均需执行）
...
- 本地 `docker run` **禁止** `--privileged`、`--network host`、build context 之外的 bind mount
- **build / run / healthcheck 任一失败必须 abort 并打印 stderr 末段，禁止继续上传**
- 镜像 tar.gz 必须 ≤ 500MB
- 收尾必须执行 `docker rmi <tag>`、`rm /tmp/Dockerfile`、`rm /tmp/showcase-image.tar.gz`（无论成功失败）
- **所有 `FROM` 引用的 Docker Hub 镜像必须加 `docker.1ms.run/` 代理前缀**：
...
| `build` 失败（前端 / docker build） | 输出 stderr 尾部并终止 |
...
| `docker run` 启动失败 | 打印 `docker logs` 末段，清理容器/镜像/Dockerfile，终止 |
| healthcheck 30s 内未命中可接受状态码 | 打印 `docker logs` 末段，清理，终止 |
...
| API 返回 `image_invalid` | 提示「镜像 tar 校验失败，请确认 docker save 流程未中断」并终止 |
| API 返回 `container_start_failed` | 透传 `data.detail`（≤200 字）并终止 |
| API 返回 `healthcheck_failed` | 提示「服务端启动后 healthcheck 失败：<detail>，请本地复跑 docker run + curl 自查」并终止 |
