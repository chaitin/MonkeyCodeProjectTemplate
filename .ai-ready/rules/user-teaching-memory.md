...
- **不记录可通过 Explore 代码获取的内容**：不要记录会被写入代码中、后续通过 Explore 代码能够了解到的内容（例如接口定义、类型声明、函数签名、模块路径等）。只记录用户对该项目的**特定行为定义**（例如如何编译、如何运行测试、使用什么包管理器、特殊的构建步骤等），这些行为定义无法从代码中直接获取，必须由用户告知
